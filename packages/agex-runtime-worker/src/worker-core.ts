/**
 * Worker core — platform-agnostic agent-execution logic.
 *
 * This module never names `self` or `parentPort`; the platform entry
 * (`worker.ts` for the browser, `worker.node.ts` for Node) supplies a
 * `WorkerPort` to `bootWorker()` at startup. See `worker-port.ts`.
 *
...
/** The platform port supplied by `bootWorker()`. Set once at startup,
 *  before any host message can arrive, so the non-null assertions in
 *  `post` / `loadModule` are safe. */
let activePort: WorkerPort | null = null

function post(msg: Worker2HostMessage): void {
  if (activePort === null) {
    throw new Error('@agex-ts/runtime-worker: worker port used before bootWorker()')
  }
  activePort.post(msg)
}

/** Dynamic-import a URL-shipped module through the active port. Browser
 *  imports any ESM URL; Node rejects remote `http(s):` URLs with a clear
 *  error. See `WorkerPort.loadModule`. */
function loadModule(url: string): Promise<unknown> {
  if (activePort === null) {
    return Promise.reject(
      new Error('@agex-ts/runtime-worker: worker port used before bootWorker()'),
    )
  }
  return activePort.loadModule(url)
}
...
// The dynamic `import()` indirection now lives behind `WorkerPort.loadModule`
// (see `loadModule` above) so the browser and Node entries can differ on
// what URL schemes they accept.
...
function __load(executeId: number, name: string): Promise<unknown> {
  ...
  if (spec !== undefined) {
    ...
    const p = (async () => {
      try {
        const mod = (await loadModule(spec.url)) as Record<string, unknown>
...
async function handleExecute(msg: Extract<Host2WorkerMessage, { type: 'execute' }>): Promise<void> {
  ...
  if (outcome.kind === 'continue' && error === null) {
    const stopSuppressing = (activePort as WorkerPort).onUnhandledRejection(
      (reason) => reason instanceof TaskSuccessSignal || reason instanceof TaskFailSignal,
    )
    try {
...
/** Boot the worker core against a platform port. The entry modules
 *  (`worker.ts` / `worker.node.ts`) call this exactly once with the
 *  browser or Node port. Registers the single message handler and tells
 *  the host the worker is ready to receive `configure` / `execute`. */
export function bootWorker(port: WorkerPort): void {
  activePort = port
  port.onMessage((msg) => {
