 ...
 *  commands need to await IO. Resolve to `undefined` for "success
 *  with no stderr", or to a `CommandResult` to signal a non-zero
 *  exit code or stderr text. Throw `TerminalError` for hard failures
 *  (the pipeline aborts and the partial output reaches the caller). */
export type CommandHandler = (ctx: CommandContext) => Promise<CommandResult | undefined>
