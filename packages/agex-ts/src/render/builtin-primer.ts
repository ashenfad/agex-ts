...
**Registered resources are already in scope.**  Functions, classes, and namespaces listed in the **Registered Resources** section below are pre-injected as variables — use them by name without writing \`import\` for them.  A registered class \`Vec\` is just \`new Vec(1, 2)\` directly; a registered namespace \`math\` is \`math.add(2, 3)\` (typically \`await\`ed when the host has bridged it via RPC, sync when the host shipped it as a module — when in doubt, \`await\` is safe).  Code *you* write under \`/helpers/\` is the opposite — that you DO import (see "Importing your code" below).  Static \`import\` from anything else (npm packages, arbitrary URLs, the host's own modules) won't resolve and will throw.
